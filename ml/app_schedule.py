...
# APScheduler setup for scheduling the flow
def start_ml_pipeline():
    """Function to trigger the Prefect flow."""
    ml_pipeline()

# Scheduler setup
scheduler = BackgroundScheduler()
scheduler.add_job(start_ml_pipeline, 'interval', minutes=5)  # Run every 15 minutes
scheduler.start()
...
if __name__ == "__main__":
    try:
        # Run Flask app
        app.run(debug=True, use_reloader=False)  # `use_reloader=False` to avoid duplicate scheduler runs
